...
export const WEEKLY_REFERRAL_TOTAL_REWARD =
  WEEKLY_REFERRAL_FIXED_COMMISSION + WEEKLY_REFERRAL_BONUS;
...
export async function ensureReferralSystemSchema(db: Db) {
  const statements = [
    sql`ALTER TABLE users ADD COLUMN signupIp VARCHAR(64) NULL`,
    sql`ALTER TABLE users ADD COLUMN signupUserAgentHash VARCHAR(64) NULL`,
    sql`ALTER TABLE referrals ADD COLUMN suspicious BOOLEAN NOT NULL DEFAULT false`,
    sql`ALTER TABLE referrals ADD COLUMN suspicionReason VARCHAR(255) NULL`,
    sql`ALTER TABLE referrals ADD COLUMN referredPremiumActivatedAt TIMESTAMP NULL`,
    sql`ALTER TABLE referrals ADD COLUMN discountApplied BOOLEAN NOT NULL DEFAULT false`,
    sql`ALTER TABLE referrals ADD COLUMN discountAmount INT NOT NULL DEFAULT 0`,
    sql`ALTER TABLE paymentTransactions ADD COLUMN durationMonths INT NOT NULL DEFAULT 1`,
    sql`ALTER TABLE paymentTransactions ADD COLUMN originalAmount INT NULL`,
    sql`ALTER TABLE paymentTransactions ADD COLUMN discountAmount INT NOT NULL DEFAULT 0`,
    sql`ALTER TABLE paymentTransactions ADD COLUMN referralId INT NULL`,
    sql`
...
export async function getPremiumPlanQuote(db: Db, userId: number, durationMonths: number) {
  ...
  const [plan] = await db
    ...
    .where(eq(premiumSubscriptions.durationMonths, durationMonths))
    .limit(1);
  ...
  const canApplyReferralDiscount =
    !!referral && !referral.suspicious && !referral.discountApplied && referral.referrerId !== userId;
  const discountAmount = canApplyReferralDiscount
    ? Math.floor((basePrice * REFERRAL_DISCOUNT_PERCENT) / 100)
    : 0;
...
export async function markReferralPremiumActivated(
  db: Db,
  ...
  await db
    .update(referrals)
    .set({ referredPremiumActivatedAt: activatedAt })
    .where(eq(referrals.id, referral.id));

  return { ...referral, referredPremiumActivatedAt: activatedAt };
}

export async function getReferralLeaderboardForWindow(
  db: Db,
  window: ReferralWeekWindow,
  limit = 10,
  referenceTime = new Date()
) {
  await ensureReferralSystemSchema(db);
  ...
  const weeklyReferrals = await db
    ...
    .where(
      and(
        eq(referrals.suspicious, false),
        gte(referrals.referredPremiumActivatedAt, window.start),
        lt(referrals.referredPremiumActivatedAt, window.end)
      )
    );
...
export async function getWeeklyReferralRewardHistory(db: Db, limit = 10) {
  ...
  const payouts = await db
    .select()
    .from(referralWeeklyRewardPayouts)
    .orderBy(desc(referralWeeklyRewardPayouts.paidAt))
    .limit(limit);
...
export async function runWeeklyReferralSettlement(db: Db, now = new Date()) {
  ...
  const [existingPayout] = await db
    .select()
    .from(referralWeeklyRewardPayouts)
    .where(eq(referralWeeklyRewardPayouts.weekKey, previousWeek.weekKey))
    .limit(1);
  ...
  const leaderboard = await getReferralLeaderboardForWindow(
    db,
    previousWeek,
    1,
    previousWeek.end
